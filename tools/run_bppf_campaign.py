...
1. gets the exact breakpoint sequence from our own HFMA solve;
2. times HFMA and RaC end-to-end (they compute the whole sequence in one
...
4. cross-checks, for every sampled midpoint, that BPPF's minimum-cut closure
   agrees with HFMA's closure at that lambda (reusing the same Oracle-2
...
caveat attached, never silently merged into the HFMA/RaC comparison tables.
"""
...
def hfma_breakpoints(pcf_solve: Path, instance: Path) -> list[Fraction]:
    completed = subprocess.run([str(pcf_solve), "--instance", str(instance), "--algorithm", "hfma"],
                                check=True, text=True, capture_output=True)
    ratios: list[Fraction] = []
    for line in completed.stdout.splitlines():
        if line.startswith("macroitem "):
            token = line.split()[3]
            p, w = token.split("/")
...
def closure_from_hfma(pcf_solve: Path, instance: Path, lam: Fraction) -> set[int]:
    completed = subprocess.run([str(pcf_solve), "--instance", str(instance), "--algorithm", "hfma"],
                                check=True, text=True, capture_output=True)
    closure: set[int] = set()
    for line in completed.stdout.splitlines():
...
def time_bppf_total(pcf_bppf_oracle: Path, instance: Path, lambdas: list[Fraction], n: int) -> tuple[int, int]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for lam in lambdas:
            ...
            expected = closure_from_hfma_cached.get((instance, lam))
            if expected is not None and bppf_closure != expected:
                mismatches += 1
    return total_ns, mismatches


closure_from_hfma_cached: dict[tuple[Path, Fraction], set[int]] = {}
...
def main() -> None:
    ...
    parser.add_argument("--max-breakpoints", type=int, default=200,
                         help="skip instances whose HFMA sequence has more sampled midpoints than this "
    ...
    for instance in sorted(arguments.instances.glob("*.pcf")):
        ...
        ratios = hfma_breakpoints(arguments.pcf_solve, instance)
        ...
        if len(lambdas) > arguments.max_breakpoints:
            ...
            continue
        for lam in lambdas:
            closure_from_hfma_cached[(instance, lam)] = closure_from_hfma(arguments.pcf_solve, instance, lam)

        hfma_times = time_ns(arguments.pcf_benchmark, instance, "hfma", arguments.repetitions, "campaign_f")
        ...
        row = {
            ...
            "hfma_median_ns": statistics.median(hfma_times),
        ...
        print(f"{instance.name}: n={n} breakpoints={len(lambdas)} "
              f"hfma={row['hfma_median_ns']}ns rac={row['rac_median_ns']}ns "
    ...
    with arguments.output.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0].keys()) if rows else
                                 ["instance", "n_nodes", "n_breakpoints_sampled", "hfma_median_ns",
