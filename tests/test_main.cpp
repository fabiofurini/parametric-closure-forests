...
void test_rac_differential() {
    ...
    for (int n = 1; n <= 100; ++n) {
        for (int repetition = 0; repetition < 20; ++repetition) {
            ...
            const auto hpac_lazy = pcf::compute_hpac_lazy(instance);
            ...
            require_well_formed(instance, hpac_lazy);
            ...
            require(same_sequence(pac, hpac_lazy), "PaC and HPaC-Lazy differ on random forest");
...
void test_rac_small_random_enumeration() {
    ...
    for (int repetition = 0; repetition < 10000; ++repetition) {
        ...
        require_matches_enumeration(instance, hpac_lazy);
        ...
        require(same_sequence(pac, hpac_lazy), "PaC and HPaC-Lazy differ on enumerated random forest");
...
void test_structured_rac_differential() {
    for (const std::string shape : {"path", "binary", "star"}) {
        for (const int n : {1, 2, 3, 4, 7, 16, 63, 128, 257}) {
            ...
            require(same_sequence(pac, hpac_lazy), "PaC and HPaC-Lazy differ on structured tree");
            ...
            for (int direction = 0; direction < (1 << static_cast<int>(edges.size())); ++direction) {
                for (int profit_code = 0; profit_code < profit_cases; ++profit_code) {
                    for (int weight_code = 0; weight_code < (1 << n); ++weight_code) {
                        ...
                        require(same_sequence(pac, hpac_lazy), "PaC and HPaC-Lazy differ on exhaustive small forest");
...
int main() {
    try {
        ...
        require(same_sequence(pac, hpac_lazy), "PaC and HPaC-Lazy differ");
