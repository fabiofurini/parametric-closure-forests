...
MacroitemSequence compute_hima(const Instance& instance) {
    ...
    if (!is_in_forest(instance)) throw std::invalid_argument("HIMA requires an in-forest");
    ...
    for (int alive = instance.n; alive > 0; --alive) {
        Entry entry{};
        do {
            if (heap.empty()) throw std::runtime_error("HIMA heap became empty");
        ...
        if (graph[u].out.empty()) {
            Macroitem macroitem = make_macroitem(instance, graph[u].original_nodes);
            if (!current || compare(macroitem.ratio(), *current) < 0) {
                current = macroitem.ratio();
                sequence.macroitems.push_back(std::move(macroitem));
            } else {
                auto& previous = sequence.macroitems.back();
